# FastAPI Test Chat API

**Тестовое задание**: API для работы с чатами и сообщениями.

---

## 🏗 Архитектура проекта

Проект разделен на логические слои:

    fastapi-chat-api/
    ├── src/
    │ ├── api/                         # Роутеры FastAPI
    │ │ ├── routers.py                 # Эндпоинты для чатов и сообщений
    │ │ └── dependencies.py            # Depends для проверки прав доступа
    │ ├── service/                     # Бизнес-логика
    │ │ ├── chat.py
    │ │ └── message.py
    │ ├── repo/                        # Репозитории для работы с БД
    │ │ ├── chat.py
    │ │ └── message.py
    │ ├── models/                      # Модели SQLAlchemy (ORM)
    │ │ ├── chat.py
    │ │ └── message.py
    │ ├── schemas/                     # Pydantic схемы валидации
    │ │ ├── chat.py
    │ │ └── message.py
    │ ├── core/                        # Основные утилиты
    │ │ ├── config.py                  # Конфигурация через Pydantic
    │ │ ├── database.py                # Подключение к базе данных
    │ │ └── logger.py                  # Настройка логирования
    │ └── init.py
    ├── migrations/                    # Alembic миграции
    ├── tests/                         # Тесты
    │ ├── conftest.py
    │ ├── test_chats.py
    │ └── test_messages.py
    ├── .env.example                   # Пример переменных окружения
    ├── Dockerfile                     # Контейнеризация приложения
    ├── docker-compose.yml             # Docker Compose конфигурация
    ├── alembic.ini                    # Alembic конфиг
    ├── pyproject.toml                 # Конфигурация проекта (Poetry)
    ├── poetry.lock                    # Зафиксированные зависимости
    └── README.md                      # Документация проекта

---
## 📦 Модели

**Chat**
- `id: int`
- `title: str`
- `created_at: datetime`

**Message**
- `id: int`
- `chat_id: int` (FK на Chat)
- `text: str`
- `created_at: datetime`

---
## 🛠 Методы API

| Метод | Путь | Body | Response |
|-------|------|------|----------|
| POST  | /chats/ | `title: str` | созданный чат |
| POST  | /chats/{id}/messages/ | `text: str` | созданное сообщение |
| GET   | /chats/{id}?limit=N |  | чат + последние N сообщений (сортировка по `created_at`) |
| DELETE| /chats/{id} |  | 204 No Content |

---
## ⚙️ Стек

- FastAPI
- SQLAlchemy + PostgreSQL
- Alembic для миграций
- Pydantic для валидации и сериализации данных
- Poetry для управления зависимостями и сборки проекта
- Docker + docker-compose
- Логирование через Python logger
- Тесты через pytest

---
## 📦 Запуск через Docker

### 1. Клонируем репозиторий
```bash
git clone https://github.com/Dafoe17/fastapi-chat-api.git
cd fastapi-auth-test
```

### 2. Создаём файл с настройками
```bash
cp .env.example .env
```

### 3. Поднять сервисы через Docker
```bash
docker-compose up --build
```

### 4. Проверка
Home: http://localhost:8000

Swagger UI: http://localhost:8000/docs

```bash
curl http://localhost:8000/ping
```

### 5. Тесты

```bash
docker-compose exec web pytest -v
```
---


